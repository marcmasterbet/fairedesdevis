...
export default function NouveauDevis() {
  ...
  const [ligneAutreQte, setLigneAutreQte] = useState('1')
  ...
  const ajouterLigneAutre = () => {
    ...
    const qte = parseFloat(ligneAutreQte) || 1
    const prixHT = parseFloat(ligneAutreMontant) || 0
    const montant = qte * prixHT
    setLignesAutres(l => [...l, { nom: ligneAutreNom, qte, prixHT, montant }])
    setLigneAutreNom('')
    setLigneAutreMontant('')
    setLigneAutreQte('1')
  }

  const totalHTBrut = lignes.reduce((s, l) => s + l.total_ht, 0) + lignesAutres.reduce((s, la) => s + la.montant, 0)
  ...
  const handleGenerer = async () => {
    ...
    const toutesLesLignes = [
      ...lignes.map(l => l.nom + '|' + (l.reference || '-') + '|' + l.quantite + '|' + l.unite + '|' + l.prix_ht.toFixed(2) + ' EUR|' + l.total_ht.toFixed(2) + ' EUR'),
      ...lignesAutres.map(la => la.nom + '|-|' + la.qte + '|unite|' + la.prixHT.toFixed(2) + ' EUR|' + la.montant.toFixed(2) + ' EUR')
    ].join('\n')
...
TOUTES LES PRESTATIONS (a mettre dans UN SEUL tableau - format: Designation|Reference|Qte|Unite|Prix HT|Total HT) :
${toutesLesLignes}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 mb-4">
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <input type="text" className="flex-1 min-w-32 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="Ex: Trajet, Materiel special..." value={ligneAutreNom} onChange={e => setLigneAutreNom(e.target.value)} />
              <input type="number" className="w-16 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="Qte" value={ligneAutreQte} onChange={e => setLigneAutreQte(e.target.value)} />
              <input type="number" className="w-24 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="Prix HT" value={ligneAutreMontant} onChange={e => setLigneAutreMontant(e.target.value)} />
            ...
            {lignesAutres.map((la, i) => (
              <div key={i} className="flex items-center gap-3 p-3 mt-2 rounded-lg border border-purple-200 bg-purple-50">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900">{la.nom}</p>
                  <p className="text-xs text-gray-400">{la.qte} x {la.prixHT.toFixed(2)} EUR</p>
                </div>
                <span className="text-sm font-semibold text-gray-900 w-20 text-right">{la.montant.toFixed(2)} EUR</span>
