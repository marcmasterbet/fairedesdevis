...
export default function VoirFacture({
  ...
  useEffect(() => {
    const init = async () => {
      const res = await fetch(`/api/facture-publique/${id}`)
      if (res.ok) {
  ...
  return (
    <main className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-3xl mx-auto mt-6 px-4 print:hidden">
        <div className="bg-white rounded-xl border border-gray-200 p-5 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Client : {facture.client_nom}</p>
            <p className="text-sm text-gray-500">{facture.client_email}</p>
          </div>
      ...
      <div className="max-w-3xl mx-auto my-6 bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="p-8" dangerouslySetInnerHTML={{ __html: facture.contenu }} />
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 pb-8 print:hidden">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex justify-between py-2">
            ...
            <strong>{Number(facture.montant_ht).toFixed(2)} €</strong>
          </div>
          <div className="flex justify-between py-2">
            <span>TVA</span>
            <strong>{Number(facture.tva).toFixed(2)} €</strong>
          </div>
          <div className="flex justify-between border-t pt-4 mt-2 text-lg font-bold">
            <span>Total TTC</span>
            <span className="text-blue-600">{Number(facture.montant_ttc).toFixed(2)} €</span>
          </div>
        </div>
      </div>
