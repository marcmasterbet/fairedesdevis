...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const nomFormate = nom ? nom.charAt(0).toUpperCase() + nom.slice(1) : devisData.client_nom
    ...
    if (estAccepte && devisData.client_email) {
      await resend.emails.send({
        ...
        html: '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px"><div style="background:#2563eb;padding:24px;border-radius:12px 12px 0 0;text-align:center"><h1 style="color:white;margin:0;font-size:24px">FaireDesDevis</h1></div><div style="background:white;padding:32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px"><p style="font-size:16px;color:#1e293b">Bonjour <strong>' + nomFormate + '</strong>,</p><p style="color:#64748b">Confirmation de votre acceptation du devis <strong>' + devisData.numero + '</strong> de <strong>' + prestaNom + '</strong>.</p><div style="background:#f0fdf4;border:1px solid #bbf7d0;padding:16px;border-radius:8px;margin:20px 0"><p style="margin:0;color:#16a34a;font-weight:bold">Devis accepte</p><p style="margin:8px 0 0;color:#64748b;font-size:13px">Montant : ' + Number(devisData.montant_ttc).toFixed(2) + ' EUR TTC</p></div><p style="color:#64748b;font-size:13px;margin-top:16px">Vous pouvez consulter et telecharger votre devis signe en cliquant sur le bouton ci-dessous :</p><a href="https://fairedesdevis.fr/signer/' + devisId + '" style="background:#2563eb;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:bold;display:inline-block;margin-top:8px">Voir mon devis signe</a><p style="color:#94a3b8;font-size:11px;margin-top:12px">Conservez cet email comme preuve d acceptation.</p></div></div>'
      })
    }
