...
export async function POST(req: NextRequest) {
  const { clientEmail, clientNom, prestataireNom, numero, montantTTC, lienFacture } = await req.json()

  try {
    await resend.emails.send({
      from: 'FaireDesDevis <noreply@fairedesdevis.fr>',
      to: clientEmail,
      subject: `Facture ${numero} de ${prestataireNom}`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px">
          ...
          <div style="background:white;padding:32px;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px">
            ...
            <p style="color:#64748b">Vous avez reçu une facture de la part de <strong>${prestataireNom}</strong>.</p>
            <div style="background:#f8fafc;border-left:4px solid #2563eb;padding:16px;border-radius:0 8px 8px 0;margin:24px 0">
              <p style="margin:0;color:#64748b;font-size:14px">Facture N°</p>
            ...
            <p style="color:#64748b">Pour consulter et télécharger votre facture, cliquez sur le bouton ci-dessous :</p>
            <div style="text-align:center;margin:32px 0">
              <a href="${lienFacture}" style="background:#2563eb;color:white;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px;display:inline-block">
                Voir ma facture →
              </a>
            </div>
            <p style="color:#94a3b8;font-size:12px;text-align:center">
              Si vous avez des questions, contactez directement <strong>${prestataireNom}</strong>.
            </p>
          </div>
